...
function executarSemWarning(callable $operacao)
{
...
function enviarEmail(string $destino, string $assunto, string $mensagem, string $headers): bool
{
    ...
    return (bool)executarSemWarning(static fn () => mail($destino, $assunto, $mensagem, $headers));
}

function obterArquivoLogPorTipo(string $tipo): string
{
    if ($tipo === 'simulacao') {
        return __DIR__ . '/storage/contatos.log';
    }

    return __DIR__ . '/storage/mensagens.log';
}
...
function registrarFormularioLocal(string $tipo, array $campos, ?string $erroMail = null): bool
{
    ...
    $arquivo = obterArquivoLogPorTipo($tipo);
    $partes = [];
...
try {
    ...
    if ($tipoFormulario === 'contato') {
        ...
        if (!$enviado) {
            if ($registradoLocalmente) {
                respostaJson('Recebemos sua mensagem e retornaremos em breve.', 200);
            }
    ...
    if ($tipoFormulario === 'simulacao') {
        ...
        respostaJson('Recebemos sua solicitação e já está entrando em análise, aguarde nosso retorno!');
    }
